 ...
 char DBFileRecord_ReadBuffer(FileRecord *fileRecord)
{
	...
	fread ((char*)g_buffer+(fileRecord->table.row_length)*n,sizeof(char)*fileRecord->table.row_length,BUFFER_SIZE,fileRecord->fP);
 ...
 char DBFileRecord_WriteBuffer(FileRecord *fileRecord)
 {
 	...
 	fseek ( fileRecord->fP, fileCursor , SEEK_SET);
 	n = fwrite ((char*)g_buffer+(fileRecord->table.row_length)*n,sizeof(char)*fileRecord->table.row_length,BUFFER_SIZE-n,fileRecord->fP);
...
char DBTable_WriteRow(Table *table,int n)
{
	...
	int tot_length =0;
	void *row = ((char*)g_buffer+n*table->row_length);
	for (i=0;i<table->header.nFields;i++)
	{
		...
		{
			case DB_LEN8:
			 	*((char  *)( (char*)row+tot_length))=table->fields[i]->buffer[n];
				tot_length+=1;
			break;

			case DB_LEN16:
				*((short  *)((char*)row+tot_length))=table->fields[i]->buffer[n];
				tot_length+=2;
			break;

			case DB_LEN32:
				*((int  *)( (char*)row+tot_length))=table->fields[i]->buffer[n];

				tot_length+=3;
			break;

			case DB_LEN64:
				*((long  *)( (char*)row+tot_length))=table->fields[i]->buffer[n];
				tot_length+=4;
			break;
...
char DBTable_ReadRow(Table *table,int i_g_buffer)
{
	...
	int tot_length=0;
	int field_length;
	void *row = ((char*)g_buffer+table->row_length*i_g_buffer);
	for (i=0;i<table->header.nFields;i++)
	{
		field_length = table->fields[i]->length;
		switch (field_length-1)
		{
			case DB_LEN8:
				table->fields[i]->buffer[i_g_buffer]=*((char  *)((char*)row+tot_length));
				tot_length+=1;
			break;

			case DB_LEN16:
				table->fields[i]->buffer[i_g_buffer] = *((short  *)((char*)row+tot_length));
				tot_length+=2;
			break;

			case DB_LEN32:
				table->fields[i]->buffer[i_g_buffer] = *((int  *)( (char*)row+tot_length));
				tot_length+=3;
			break;

			case DB_LEN64:		
				table->fields[i]->buffer[i_g_buffer] = *((long  *)( (char*)row+tot_length));
				tot_length+=4;
			break;
